...
import {useEditESGModal} from './ESGModalContext'
...
export default function EditModal({category}: ESGModalProps) {
  const {
    ...
    chartToEdit // ⭐ 수정할 차트 정보 (Context에서 받아옴)
  } = useEditESGModal()
  ...
  // ⭐ 수정 대상 차트가 있으면 상태 초기화
  useEffect(() => {
    if (chartToEdit) {
      const {fields, years, chartType, title} = chartToEdit

      setYears(years)
      setTitle(title)
      setChartType(chartType)
      setSelectedRows(fields.map((_, i) => i))
      setColorSet(fields.map(field => field.color))

      const transformedRows: Row[] = fields.map((field, idx) => ({
