...
import Chartpage from '../ui/charts/chartEx' // Chartpage 컴포넌트 가져오기

const ItemType = {
  BOX: 'box' // 드래그 앤 드롭에서 사용할 아이템 타입 정의
}

const apiUrl = 'https://localhost:4000/api/columns' // 백엔드 API URL 정의
...
export default function Environmental() {
  ...
  useEffect(() => {
    const fetchGridItems = async () => {
      try {
        ...
        setGridItems(response.data) // 가져온 데이터를 상태에 저장
      } catch (error) {
        console.error('컬럼 데이터 로드 실패:', error) // 에러 로그 출력
      }
    }
    fetchGridItems() // 데이터 로드 함수 호출
  }, [])

  const sendColumnsToBackend = async updatedItems => {
    try {
      await axios.post(`${apiUrl}/update`, {
        columns: updatedItems // 업데이트된 컬럼 데이터를 백엔드로 전송
      })
    } catch (error) {
      console.error('컬럼 업데이트 실패:', error) // 에러 로그 출력
    }
  }
  ...
  const handleDeleteItem = () => {
    ...
    sendColumnsToBackend(updatedItems) // 삭제 후 백엔드로 데이터 전송
  ...
  const handleAddItem = () => {
    ...
    setGridItems(updatedItems) // 상태 업데이트
    sendColumnsToBackend(updatedItems) // 추가 후 백엔드로 데이터 전송
    setIsModalOpen(false) // 일반 모달 닫기
  }
  ...
  const moveItem = (dragIndex, hoverIndex) => {
    ...
    setGridItems(updatedItems) // 상태 업데이트
    sendColumnsToBackend(updatedItems) // 순서 변경 후 백엔드로 데이터 전송
  }
